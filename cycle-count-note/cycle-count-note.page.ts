...
export class CycleCountNotePage extends PageBase {
	...
	preLoadData(event) {
		...
		Promise.all([this.env.getStatus('CycleCountStatus')]).then((values: any) => {
	...
	loadData(event) {
		...
		this.pageProvider.apiPath.getList.url = function () {
			return ApiSetting.apiDomain('WMS/CycleCount/List');
		};
	...
	loadCycleCountNote(i) {
		...
		let apiPath = {
			method: 'GET',
			url: function () {
				return ApiSetting.apiDomain('WMS/CycleCount/CycleCountDetailList/');
			},
		};

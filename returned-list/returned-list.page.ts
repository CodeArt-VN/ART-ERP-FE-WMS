...
export class ReturnedLlistPage extends PageBase {
    ...
    loadReturnedLlist() {
        ...
        if (!selected.length) {
            this.env.showTranslateMessage('erp.app.pages.wms.returned-list.message.select-picking-vehicle', 'warning');
            return;
        }

        if (this.submitAttempt) {
            this.env.showTranslateMessage('erp.app.pages.wms.returned-list.message.creating');
            return;
        }
            ...
            this.pageProvider.commonService.connect(apiPath.method, apiPath.url(), docQuery).toPromise()
                .then((resp: any) => {
                    ...
                    if (err.message != null) {
                        ...
                        this.env.showTranslateMessage('erp.app.pages.wms.returned-list.message.can-not-create-picking-list','danger');
                    }
                    this.submitAttempt = false;
                    if (loading) loading.dismiss();
                });
    ...
    confirmInboundReturn() {
        ...
        this.pageProvider.commonService.connect(apiPath.method, apiPath.url(ids), this.query)
            .toPromise()
            .then((resp: any) => {
                this.env.showTranslateMessage('erp.app.pages.wms.returned-list.message.confirm-inbound-return', 'success');
                this.sheets = [];
                this.refresh();
                this.submitAttempt = false;
            }).catch(err => {
                if (err.message != null) {
                    ...
                    this.env.showTranslateMessage('erp.app.pages.wms.returned-list.message.can-not-create-list','danger');
                }
                this.refresh();
                this.submitAttempt = false;
            });

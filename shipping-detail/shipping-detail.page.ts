...
import { BRA_BranchProvider, CRM_ContactProvider, HRM_StaffProvider, SYS_SchemaProvider, WMS_ItemProvider, WMS_ShippingDetailProvider, WMS_ShippingProvider } from 'src/app/services/static/services.service';
...
export class ShippingDetailPage extends PageBase {
    ...
    changeStatus() {
        ...
        this.env.showLoading('Vui lòng chờ load dữ liệu...', this.pageProvider.commonService.connect('GET', 'WMS/Shipping/ChangeStatus/', this.query).toPromise())
        .then((result: any) => {
            this.refresh();
        })
        this.query.Id = undefined;
    ...
    deleteItems() {
        if (this.pageConfig.canDelete) {
            ...
            this.env.showPrompt('Bạn chắc muốn xóa ' + itemsToDelete.length + ' đang chọn?', null, 'Xóa ' + itemsToDelete.length + ' dòng').then(_ => {
                this.env.showLoading('Xin vui lòng chờ trong giây lát...', this.shippingDetailService.delete(itemsToDelete))
                    .then(_ => {

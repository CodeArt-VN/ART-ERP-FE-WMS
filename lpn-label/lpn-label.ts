	...
	receiptSearch() {
		...
		this.receiptList$ = concat(
			...
			this.receiptListInput$.pipe(
				...
				switchMap((term) =>
					this.receiptProvider.search({ IDBranch: this.env.selectedBranch, Take: 20, Skip: 0, Id: term ? term : '' }).pipe(
						catchError(() => of([])), // empty list on error
 ...
 */
	...
	GetLabel() {
		this.env
			.showLoading('Please wait for a few moments', this.pageProvider.commonService.connect('GET', 'WMS/LicencePlateNumber/GetLabel', this.query).toPromise())
			.then((data: any) => {
				if (data) {

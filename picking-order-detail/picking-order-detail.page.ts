...
export class PickingOrderDetailPage extends PageBase {
	...
	addField(field: any, markAsDirty = false) {
		...
		let group = this.formBuilder.group({
			...
			QuantityPicked: new FormControl({
				value: field?.QuantityPicked,
			...
			LotLPNLocation: new FormControl({
				value: field?.Lot + '-' + field?.LPN + '-' + field?.FromLocation,
				disabled: field?.Status != 'Active' ? true : false,
			}),
	...
	closePick() {
		...
		this.env.showPrompt('Bạn có chắc muốn đóng tất cả các sản phẩm lấy hàng?', null, 'Đóng lấy hàng').then((_) => {
			this.env
				.showLoading('Please wait for a few moments', this.pageProvider.commonService.connect('GET', 'WMS/Picking/ClosePicking/', this.query).toPromise())
	...
	allocatePicking() {
		...
		this.env
			.showLoading('Please wait for a few moments', this.pageProvider.commonService.connect('GET', 'WMS/Picking/AllocatePicking/', this.query).toPromise())
			.then((result: any) => {
				this.refresh();
			})
			.catch((err) => {
	...
	changedFromLocationID(valueNew, group, index) {
		...
		if (valueNew) {
			...
			let LotLPNLocation = group.controls.Lot.value + '-' + group.controls.LPN.value + '-' + group.controls.FromLocation.value;
	...
	changeQuantity(group) {
		...
		if (dataRow.Quantity > 0) {
			let lotLPNLocationSelected = dataRow._LotLPNLocationsDataSource.find((f) => f.IDLot == dataRow.Lot && f.IDLPN == dataRow.LPN && f.IDLocation == dataRow.FromLocation);
	...
	copyPickingOrderDetail(group) {
		let itemAdd = {
			...
			Status: 'Active',
			LotLPNLocation: group.controls.Lot.value + '-' + group.controls.LPN.value + '-' + group.controls.FromLocation.value,
	...
	openLotLPNLocations(group) {
		...
		let lotLPNLocationsExist = groups.controls.filter((d) => d.get('IDParent').value == group.get('IDParent').value && d != group).map((e) => e.get('LotLPNLocation').value);
	...
	removeField(fg, j) {
		...
		if (itemToDelete.Id) {
			this.env.showPrompt('Bạn có chắc muốn xóa không?', null, 'Xóa 1 dòng').then((_) => {
				this.env
					.showLoading('Please wait for a few moments', this.commonService.connect('GET', 'WMS/Picking/DeleteDetails', { Id: itemToDelete.Id }).toPromise())
					.then((_) => {
						groups.removeAt(j);
					});
			});
				...
				.then((_) => {
					this.env
						.showLoading(
							...
							this.commonService.connect('GET', 'WMS/Picking/DeleteDetails', { Id: itemsToDelete.map((d) => d.Id) }).toPromise()
						)
						.then((_) => {

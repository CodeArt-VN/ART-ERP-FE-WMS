...
export class BatchPickingPage extends PageBase {
								...
								for (let j = 0; j < c.Orders.length; j++) {
									...
									o.OrderLines.forEach((l) => {
										...
										if (l.Item.UoM.Id == l.Item.BaseUoM.Id || l.Item.UoM.Id == l.Item.InventoryUoM.Id) {
											...
											if (calcUom.BaseQuantity >= findItem.InventoryUoMBaseQuantity && findItem.InventoryUoMBaseQuantity > 0) {
												calcUom.InventoryQuantity +=
													(calcUom.BaseQuantity - (calcUom.BaseQuantity % findItem.InventoryUoMBaseQuantity)) / findItem.InventoryUoMBaseQuantity;
												calcUom.BaseQuantity = calcUom.BaseQuantity % findItem.InventoryUoMBaseQuantity;
											}

											let qdThung = parseInt(
												((calcUom.BaseQuantity * l.Item.InventoryUoM.AlternativeQuantity) / l.Item.InventoryUoM.BaseQuantity).toString()
											);
											if (qdThung > 0) {
										...
										if (l.Item.UoM.Id == l.Item.BaseUoM.Id || l.Item.UoM.Id == l.Item.InventoryUoM.Id) {
											...
											let qdThung = parseInt(
												((calcWarehouseUOM.BaseQuantity * l.Item.InventoryUoM.AlternativeQuantity) / l.Item.InventoryUoM.BaseQuantity).toString()
											);
											if (qdThung > 0) {
	...
	createOutbound() {
		...
		this.env
			.showLoading('Please wait for a few moments', this.outboundProvider.commonService.connect('POST', 'WMS/OutboundOrder/CreateOutboundFromShipments/', obj).toPromise())
			.then((result: any) => {
				if (result) {

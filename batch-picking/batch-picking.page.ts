...
export class BatchPickingPage extends PageBase {
    ...
    loadBatchPicking() {
        if (this.submitAttempt) {
            this.env.showTranslateMessage('erp.app.pages.wms.batch-picking.message.importing');
            return;
        }
        ...
        if (!selected.length) {
            this.env.showTranslateMessage('erp.app.pages.wms.batch-picking.message.importing', 'warning');
            return;
        }
            ...
            this.pageProvider.commonService.connect(apiPath.method, apiPath.url(), docQuery).toPromise()
                ...
                .catch(err => {
                    if (err.message != null) {
                        ...
                        this.env.showTranslateMessage('erp.app.pages.wms.batch-picking.message.can-not-create-list','danger');
                    }
                    this.submitAttempt = false;
                    if (loading) loading.dismiss();
                });

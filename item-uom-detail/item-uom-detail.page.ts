...
export class ItemUomDetailPage extends PageBase {
    ...
    scanQRCode() {
        if (!Capacitor.isPluginAvailable('BarcodeScanner') || Capacitor.platform == 'web'){
            this.env.showTranslateMessage('erp.app.pages.wms.item-uom-detail.message.mobile-only', 'warning');
            return;
        }
    ...
    disableProduct(){
        ...
        this.itemProvider.disable(prds, this.product.IsDisabled).then(resp=>{
            this.env.showTranslateMessage('erp.app.pages.wms.item-uom-detail.message.save-change', 'success');

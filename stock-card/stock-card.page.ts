...
import { CRM_ContactProvider, WMS_ItemProvider, WMS_TransactionProvider } from 'src/app/services/static/services.service';
...
export class StockCardPage extends PageBase {
	...
	patchValue(data) {
		this.item = {};
		//    this.sheets = [];
		this.subItem.SplitOpeningQuantity = this.subItem._SplitOpeningQuantity.map((s) => s.Quantity + ' ' + s.UoMName).join(this.subItem.OpeningQuantity < 0 ? ' - ' : ' + ');
		this.item.data = [
			...data

...
export class PutawayStrategyDetailPage extends PageBase {
	...
	loadedData(event?: any, ignoredFromGroup?: boolean): void {
		...
		if (this.formGroup.get('IDBranch').value) {
			...
			// })
			Promise.all([this.zoneService.read(queryZoneandLocation), this.locationService.read(queryZoneandLocation)]).then((values: any) => {
	...
	IDBranchChange() {
		...
		if (
			...
			groups.controls.some((g) => g.get('FromZone').value || g.get('ToZone').value || g.get('FromLocation').value || g.get('ToLocation').value)
		) {
			this.env

...
export class PickingOrderPage extends PageBase {
  ...
  approve() {
    ...
    this.env
      .showPrompt({ code: 'Bạn có chắc muốn duyệt {{value}} đang chọn?', value: this.selectedItems.length }, null, {
        code: 'Duyệt {{value1}} dòng',
        value1: this.selectedItems.length,
      })
      .then((_) => {
        this.pageProvider.commonService
          .connect('POST', 'WMS/picking-order/Approve', obj)
          .toPromise()
          .then((_) => {
            this.refresh();
          })
  ...
  disapprove() {
    ...
    this.env
      .showPrompt(
        { code: 'Bạn có chắc muốn bỏ duyệt {{value}} đang chọn?', value: { value: this.selectedItems.length } },
        null,
        { code: 'Bỏ duyệt {{value1}} dòng', value: { value1: this.selectedItems.length } },
      )
      .then((_) => {
        this.pageProvider.commonService
          .connect('POST', 'WMS/picking-order/Disapprove', obj)
          .toPromise()
          .then((_) => {
            this.refresh();
          })

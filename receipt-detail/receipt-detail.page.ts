    ...
    changedToLocation(group, e) {
        if (e) {
            ...
            this.receiptPalletizationProvider.save({ Id: group.controls.Id.value, ToLocation: group.controls.ToLocation.value }).then(resp => {
                this.env.showTranslateMessage('erp.app.pages.wms.receipt.message.update-coordinate-complete', 'success');
            ...
            this.pageProvider['importDetail'](event.target.files[0], this.id)
                .then((resp: any) => {
                    ...
                    if (resp.ErrorList && resp.ErrorList.length) {
                        ...
                        this.env.publishEvent({ Code: this.pageConfig.pageName });
                        this.env.showTranslateMessage('erp.app.pages.wms.receipt.message.import-complete', 'success');
                    }
                })
                .catch(err => {

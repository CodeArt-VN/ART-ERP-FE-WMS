...
export class ReceiptDetailPage extends PageBase {
	...
	changeLottable5(group) {
		...
		if (itemValue.Expiry && itemValue.ExpiryUnit && group.controls.Lottable5.value) {
			...
			let lot6Value = null;
			if (itemValue.ExpiryUnit == 'Year') lot6Value = lib.dateFormat(lot5Value.setFullYear(lot5Value.getFullYear() + itemValue.Expiry));
			else if (itemValue.ExpiryUnit == 'Month') lot6Value = lib.dateFormat(lot5Value.setMonth(lot5Value.getMonth() + itemValue.Expiry));
			else if (itemValue.ExpiryUnit == 'Date') lot6Value = lib.dateFormat(lot5Value.setDate(lot5Value.getDate() + itemValue.Expiry));
